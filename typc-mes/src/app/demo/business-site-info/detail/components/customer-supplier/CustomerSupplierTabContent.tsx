...
export default function CustomerSupplierTabContent({
  ...
  // transportation 상태를 배열로 다루도록 핸들러 수정
  const handleTransportationChange = (
    section: 'customer' | 'supplier' | 'wasteProcessor',
    ...
    switch (section) {
        ...
        setCustomers(
          customers.map((c) =>
            c.id === selectedCustomerId
              ? { ...c, transportation: transportations }
              : c,
          ),
        );
        break;
        ...
        setSuppliers(
          suppliers.map((s) =>
            s.id === selectedSupplierId
              ? { ...s, transportation: transportations }
              : s,
          ),
        );
        break;
      case 'wasteProcessor':
        ...
        setWasteProcessors(
          wasteProcessors.map((w) =>
            w.id === selectedWasteProcessorId
              ? { ...w, transportation: transportations }
              : w,
          ),
        );
        break;
  ...
  const getSelectedTransportations = (
    section: 'customer' | 'supplier' | 'wasteProcessor',
  ): string[] => {
    switch (section) {
      case 'customer':
        return (
          customers.find((c) => c.id === selectedCustomerId)?.transportation ||
          []
        );
      case 'supplier':
        return (
          suppliers.find((s) => s.id === selectedSupplierId)?.transportation ||
          []
        );
      case 'wasteProcessor':
        return (
          wasteProcessors.find((w) => w.id === selectedWasteProcessorId)
            ?.transportation || []
        );
      default:
        return [];
  ...
  const toggleTransportation = (
    ...
    const currentTransportations = getSelectedTransportations(section);
    const newTransportations = currentTransportations.includes(transport)
      ? currentTransportations.filter((t) => t !== transport)
      : [...currentTransportations, transport];
    handleTransportationChange(section, newTransportations);
  };
  ...
  const removeTransportation = (
    ...
    const currentTransportations = getSelectedTransportations(section);
    const newTransportations = currentTransportations.filter(
      (t) => t !== transport,
    );
    handleTransportationChange(section, newTransportations);
  };
  ...
  const handleSaveCustomers = () => {
    ...
    const isValid = customers.every(
      (customer) =>
        customer.name.trim() !== '' && customer.transportation.length > 0,
    );
  ...
  const handleSaveSuppliers = () => {
    ...
    const isValid = suppliers.every(
      (supplier) =>
        supplier.name.trim() !== '' && supplier.transportation.length > 0,
    );
  ...
  const handleSaveWasteProcessors = () => {
    ...
    const isValid = wasteProcessors.every(
      (wasteProcessor) =>
        ...
        wasteProcessor.transportation.length > 0,
    );
      ...
      <div className="mb-6 rounded-lg bg-white p-6 shadow-md">
        ...
        {!customersCollapsed && useCustomers && (
          <CompanyItemDetail
            ...
            selectedTransportations={getSelectedTransportations('customer')}
            isTransportationExpanded={isTransportationExpanded}
            toggleTransportationExpanded={() =>
              setIsTransportationExpanded(!isTransportationExpanded)
            }
            toggleTransportation={(transport) =>
              toggleTransportation('customer', transport)
            }
            removeTransportation={(transport) =>
              removeTransportation('customer', transport)
            }
          />
        )}
      ...
      <div className="mb-6 rounded-lg bg-white p-6 shadow-md">
        ...
        {!suppliersCollapsed && useSuppliers && (
          <CompanyItemDetail
            ...
            transportationOptions={transportationOptions}
            selectedTransportations={getSelectedTransportations('supplier')}
            isTransportationExpanded={isTransportationExpanded}
            toggleTransportationExpanded={() =>
              setIsTransportationExpanded(!isTransportationExpanded)
            }
            toggleTransportation={(transport) =>
              toggleTransportation('supplier', transport)
            }
            removeTransportation={(transport) =>
              removeTransportation('supplier', transport)
            }
          />
        )}
      ...
      <div className="mb-6 rounded-lg bg-white p-6 shadow-md">
        ...
        {!wasteProcessorsCollapsed && useWasteProcessors && (
          <CompanyItemDetail
            ...
            transportationOptions={transportationOptions}
            selectedTransportations={getSelectedTransportations(
              'wasteProcessor',
            )}
            isTransportationExpanded={isTransportationExpanded}
            toggleTransportationExpanded={() =>
            ...
            toggleTransportation={(transport) =>
              toggleTransportation('wasteProcessor', transport)
            }
            removeTransportation={(transport) =>
              removeTransportation('wasteProcessor', transport)
            }
          />
        )}
