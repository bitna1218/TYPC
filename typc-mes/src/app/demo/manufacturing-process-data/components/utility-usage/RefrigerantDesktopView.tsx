...
import {
  ...
  ResourceTypeInfo,
  UnitProcessInfo,
} from '../common/ResourceUsageTabContent';
import UnitProcessSelectionModal from '../common/UnitProcessSelectionModal';

// 냉매 아이템 인터페이스 확장 (CAS No. 추가)
export interface RefrigerantItem extends ResourceItem {
  casNo: string; // CAS 번호 추가
}

interface RefrigerantDesktopViewProps {
  resourceData: RefrigerantItem[];
...
const RefrigerantDesktopView: React.FC<RefrigerantDesktopViewProps> = ({
  resourceData,
  resourceTypes,
  unitProcesses,
  resourceLabel = '냉매',
  ...
  return (
    <div>
      <div className='flex justify-between items-center mb-4'>
        <h3 className='text-lg font-medium text-gray-700'>{resourceLabel} 충진량 목록</h3>
        <button
          type='button'
              ...
              <tr key={item.id} className='hover:bg-gray-50 align-top'>
                ...
                <td className='p-1 align-middle border border-gray-300'>
                  <input
                    ...
                    value={item.casNo || ''}
                    onChange={(e) =>
                      onFieldChange(item.id, 'casNo', e.target.value)
                    }
...
export default RefrigerantDesktopView; 