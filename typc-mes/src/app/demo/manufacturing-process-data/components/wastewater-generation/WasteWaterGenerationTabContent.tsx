...
import WasteWaterGenerationDesktopView from './WasteWaterGenerationDesktopView';
import WasteWaterGenerationMobileView from './WasteWaterGenerationMobileView';
import type { UnitProcessAllocation } from '../common/ResourceUsageTabContent';

// 폐수 항목 타입 정의
export interface WasteWaterItem {
  id: string;
  wasteWaterName: string;
...
// 더미 데이터 - 폐수 구분
const DUMMY_WASTEWATER_TYPES = [
  { id: 'general', name: '일반폐수' },
  { id: 'designated', name: '지정폐수' },
];

// 더미 데이터 - 처리 방법
const DUMMY_TREATMENT_METHODS = [
  { id: 'treatment', name: '처리' },
  { id: 'discharge', name: '방류' },
  { id: 'reuse', name: '재이용' },
  { id: 'other', name: '기타' },
];
...
interface WasteWaterGenerationTabContentProps {
  siteId?: string;
}

const WasteWaterGenerationTabContent: React.FC<
  WasteWaterGenerationTabContentProps
> = ({ siteId }) => {
  // 상태 관리
  const [wasteWaterData, setWasteWaterData] = useState<WasteWaterItem[]>([]);
  ...
  useEffect(() => {
    ...
    if (wasteWaterData.length === 0) {
      const initialItem: WasteWaterItem = {
        id: `wastewater_1`,
        wasteWaterName: '',
        wasteWaterType: DUMMY_WASTEWATER_TYPES[0]?.id || '',
      ...
      setWasteWaterData([initialItem]);
    }
  }, [siteId, wasteWaterData.length]);

  // 할당 데이터 업데이트
  useEffect(() => {
    if (allocationSettings.enabled) {
      const newAllocations = wasteWaterData
        .filter((item) => item.unitProcesses.length > 0)
        .map((item) => {
          return {
            resourceId: item.id,
            resourceName: item.wasteWaterName || '폐수',
  ...
  }, [allocationSettings.enabled, allocationSettings.method, wasteWaterData]);

  // 폐수 항목 추가
  const handleAddWasteWaterItem = () => {
    const newId =
    ...
    setWasteWaterData((prev) => [...prev, newItem]);
  };

  // 폐수 항목 삭제
  const handleDeleteWasteWaterItem = (id: string) => {
    if (wasteWaterData.length <= 1) {
      alert('최소 1개 이상의 폐수 발생량 항목이 필요합니다.');
      return;
    }

    if (window.confirm('선택한 폐수 발생량 항목을 삭제하시겠습니까?')) {
  ...
  const handleMonthlyGenerationChange = (
    ...
    setWasteWaterData((prev) =>
      prev.map((item) => {
        if (item.id === id) {
          const updatedMonthlyGeneration = item.monthlyGeneration.map(
            (monthData) =>
  ...
  // 저장 핸들러
  const handleSave = () => {
    console.log('폐수 발생량 데이터:', wasteWaterData);
    console.log('할당 데이터:', allocations);
    alert('폐수 발생량 데이터가 저장되었습니다.');
  };
        ...
        {allocationSettings.method === '비율 직접 입력' && (
          <div className='mb-4 flex items-center space-x-4'>
            ...
            <div className='bg-yellow-100 px-3 py-1 rounded text-sm font-medium'>
              각 단위공정별 폐수 발생 비율을 직접 입력합니다. 합계는 100%가
              되어야 합니다.
            </div>
          </div>
        )}
          ...
          {allocations.length === 0 && (
            <div className='bg-yellow-50 p-4 rounded-md'>
              <p className='text-yellow-700 text-sm'>
                단위공정이 선택된 폐수 항목이 없습니다. 항목별로 단위공정을
                선택해주세요.
              </p>
            </div>
          )}
  ...
  const viewProps = {
    wasteWaterData,
    wasteWaterTypes: DUMMY_WASTEWATER_TYPES,
    treatmentMethods: DUMMY_TREATMENT_METHODS,
    unitProcesses: DUMMY_UNIT_PROCESSES,
    units: DUMMY_UNITS,
    onAddWasteWaterItem: handleAddWasteWaterItem,
    onDeleteWasteWaterItem: handleDeleteWasteWaterItem,
  ...
  return (
    <div className='bg-white rounded-lg shadow-md p-6'>
      ...
      <div className='flex justify-between items-center mb-4'>
        <h2 className='text-xl font-semibold text-gray-800'>8. 폐수 발생량</h2>
        <div className='flex items-center space-x-4'>
          ...
          <div className='flex items-center space-x-2'>
            ...
            <Tooltip text='폐수 발생량을 단위공정별로 할당하는 방법을 선택합니다.'>
      ...
      <div className='mb-6 space-y-2'>
        <div className='p-4 bg-green-50 border border-green-200 rounded-md'>
          <p className='text-green-700 text-sm'>
            폐수 발생량 데이터를 입력하세요. 폐수 종류별로 월별 발생량을
            입력하세요.
          </p>
        </div>
        {allocationSettings.enabled &&
      ...
      {/* 모바일/데스크톱 뷰 전환 */}
      {isMobileView ? (
        <WasteWaterGenerationMobileView {...viewProps} />
      ) : (
        <WasteWaterGenerationDesktopView {...viewProps} />
      )}
...
export default WasteWaterGenerationTabContent;
