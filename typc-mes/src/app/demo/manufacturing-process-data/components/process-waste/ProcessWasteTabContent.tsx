...
import ProcessWasteDesktopView from './ProcessWasteDesktopView';
import ProcessWasteMobileView from './ProcessWasteMobileView';
import type { UnitProcessAllocation } from '../common/ResourceUsageTabContent';

// 공정폐가스 항목 타입 정의
export interface ProcessWasteItem {
  id: string;
  wasteName: string; // 폐가스명
  density: string; // 밀도
  ...
  // 추가 정보 테이블 데이터
  heatValue: string; // 순발열량 (TJ/ton)
  emissionFactor: string; // 배출계수 (kgCO₂/ton)
}
...
interface ProcessWasteTabContentProps {
  siteId?: string;
}

const ProcessWasteTabContent: React.FC<ProcessWasteTabContentProps> = ({ siteId }) => {
  // 상태 관리
  const [processWasteData, setProcessWasteData] = useState<ProcessWasteItem[]>([]);
  ...
  useEffect(() => {
    ...
    // 초기 데이터가 없는 경우 생성
    if (processWasteData.length === 0) {
      const initialItem: ProcessWasteItem = {
        id: `processwaste_1`,
        wasteName: '',
        density: '',
        unit: 'Nm3',
      ...
      setProcessWasteData([initialItem]);
    }
  }, [siteId, processWasteData.length]);

  // 할당 데이터 업데이트
  useEffect(() => {
    if (allocationSettings.enabled) {
      const newAllocations = processWasteData
        .filter((item) => item.unitProcesses.length > 0)
        .map((item) => {
          return {
            resourceId: item.id,
            resourceName: item.wasteName || '공정폐가스',
  ...
  }, [allocationSettings.enabled, allocationSettings.method, processWasteData]);

  // 공정폐가스 항목 추가
  const handleAddProcessWasteItem = () => {
    const newId =
      processWasteData.length > 0
        ? 'processwaste_' +
          (Math.max(
            ...processWasteData.map((item) =>
    ...
    const newItem: ProcessWasteItem = {
      id: newId,
      wasteName: '',
      density: '',
      unit: 'Nm3',
    ...
    setProcessWasteData((prev) => [...prev, newItem]);
  };

  // 공정폐가스 항목 삭제
  const handleDeleteProcessWasteItem = (id: string) => {
    if (processWasteData.length <= 1) {
      alert('최소 1개 이상의 공정폐가스 사용량 항목이 필요합니다.');
      return;
    }

    if (window.confirm('선택한 공정폐가스 사용량 항목을 삭제하시겠습니까?')) {
      setProcessWasteData((prev) => prev.filter((item) => item.id !== id));
    }
  };
  ...
  const handleFieldChange = (
    id: string,
    field: keyof ProcessWasteItem,
    value: string | string[] | number
  ) => {
    setProcessWasteData((prev) =>
      prev.map((item) => {
        if (item.id === id) {
  ...
  const handleMonthlyUsageChange = (
    ...
    setProcessWasteData((prev) =>
      prev.map((item) => {
        if (item.id === id) {
          const updatedMonthlyUsage = item.monthlyUsage.map(
            (monthData) =>
  ...
  // 저장 핸들러
  const handleSave = () => {
    console.log('공정폐가스 사용량 데이터:', processWasteData);
    console.log('할당 데이터:', allocations);
    alert('공정폐가스 사용량 데이터가 저장되었습니다.');
  };
        ...
        {allocationSettings.method === '비율 직접 입력' && (
          <div className='mb-4 flex items-center space-x-4'>
            ...
            <div className='bg-yellow-100 px-3 py-1 rounded text-sm font-medium'>
              각 단위공정별 공정폐가스 사용 비율을 직접 입력합니다. 합계는 100%가
              되어야 합니다.
            </div>
          </div>
        )}
          ...
          {allocations.length === 0 && (
            <div className='bg-yellow-50 p-4 rounded-md'>
              <p className='text-yellow-700 text-sm'>
                단위공정이 선택된 공정폐가스 항목이 없습니다. 항목별로 단위공정을
                선택해주세요.
              </p>
            </div>
          )}
  ...
  const viewProps = {
    ...
    onAddProcessWasteItem: handleAddProcessWasteItem,
    onDeleteProcessWasteItem: handleDeleteProcessWasteItem,
  ...
  return (
    <div className='bg-white rounded-lg shadow-md p-6'>
      ...
      <div className='flex justify-between items-center mb-4'>
        <h2 className='text-xl font-semibold text-gray-800'>10. 공정폐가스 사용량</h2>
        <div className='flex items-center space-x-4'>
          ...
          <div className='flex items-center space-x-2'>
            ...
            <Tooltip text='공정폐가스 사용량을 단위공정별로 할당하는 방법을 선택합니다.'>
      ...
      <div className='mb-6 space-y-2'>
        <div className='p-4 bg-green-50 border border-green-200 rounded-md'>
          <p className='text-green-700 text-sm'>
            공정폐가스 사용량 데이터를 입력하세요. 폐가스 종류별로 월별 사용량과 관련 정보를
            입력하세요.
          </p>
        </div>
        {allocationSettings.enabled &&
      ...
      {/* 모바일/데스크톱 뷰 전환 */}
      {isMobileView ? (
        <ProcessWasteMobileView {...viewProps} />
      ) : (
        <ProcessWasteDesktopView {...viewProps} />
      )}
...
export default ProcessWasteTabContent;
