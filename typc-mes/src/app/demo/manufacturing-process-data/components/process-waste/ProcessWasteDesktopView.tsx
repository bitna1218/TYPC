...
import { ProcessWasteItem } from './ProcessWasteTabContent';
import UnitProcessSelectionModal from '../common/UnitProcessSelectionModal';

interface ProcessWasteDesktopViewProps {
  processWasteData: ProcessWasteItem[];
  unitProcesses: { id: string; name: string }[];
  onAddProcessWasteItem: () => void;
  onDeleteProcessWasteItem: (id: string) => void;
  onFieldChange: (
    id: string,
...
const ProcessWasteDesktopView: React.FC<ProcessWasteDesktopViewProps> = ({
  processWasteData,
  unitProcesses,
  onAddProcessWasteItem,
  onDeleteProcessWasteItem,
  onFieldChange,
  ...
  const openUnitProcessModal = (itemId: string) => {
    const item = processWasteData.find((item) => item.id === itemId);
    if (item) {
            ...
            {processWasteData.map((item) => (
              <tr key={item.id} className='hover:bg-gray-50 align-top'>
                ...
                <td className='px-2 py-2 text-center align-middle border border-gray-300'>
                  <button
                    type='button'
                    onClick={() => onDeleteProcessWasteItem(item.id)}
                    className='text-red-500 hover:text-red-700 p-1 rounded-md hover:bg-red-50'
                    aria-label='항목 삭제'
                    disabled={processWasteData.length <= 1}
                  >
                    <FaTrash size={14} />
                  </button>
                </td>
      ...
      <div className='mt-8'>
        ...
        <div className='overflow-x-auto shadow-sm border border-gray-200 rounded-lg'>
          <table className='min-w-full border-collapse'>
            ...
            <tbody className='bg-white text-xs divide-y divide-gray-200'>
              {processWasteData.map((item) => (
                <tr key={`additional-${item.id}`} className='hover:bg-gray-50 align-top'>
                  <td className='px-3 py-2 text-gray-700 align-middle border border-gray-300'>
                    {item.wasteName || <span className="text-gray-400 italic">미입력</span>}
                  </td>
                  <td className='px-3 py-2 text-gray-700 align-middle border border-gray-300 text-center'>
                    {item.unit === 'ton' ? 'ton' : item.unit === 'Nm3' ? 'm³' : item.unit}
                  </td>
                  <td className='p-1 align-middle border border-gray-300'>
                    <input
                      type='text'
                  ...
                  <td className='p-1 align-middle border border-gray-300'>
                    <input
                      ...
                      onChange={(e) => 
                        onFieldChange(item.id, 'heatValue', e.target.value)
                      }
                      className='w-full py-1.5 px-2 text-xs border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 text-right'
                      placeholder='순발열량 입력'
                    />
                  ...
                  <td className='p-1 align-middle border border-gray-300'>
                    <input
                      ...
                      onChange={(e) => 
                        onFieldChange(item.id, 'emissionFactor', e.target.value)
                      }
                      className='w-full py-1.5 px-2 text-xs border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 text-right'
                      placeholder='배출계수 입력'
                    />
...
export default ProcessWasteDesktopView; 