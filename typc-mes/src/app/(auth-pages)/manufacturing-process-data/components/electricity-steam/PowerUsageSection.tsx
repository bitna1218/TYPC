...
import PowerUsageMobileView from './PowerUsageMobileView';
import PowerUsageDesktopView from './PowerUsageDesktopView';
...
const PowerUsageSection: React.FC<PowerUsageSectionProps> = ({
  powerUsageData,
  setPowerUsageData,
  measurementData,
  setMeasurementData,
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') { // SSR 안전장치
      checkMobileView();
  ...
  const handleAddPowerUsageItem = () => {
    const newId =
      powerUsageData.length > 0
        ? 'power_' + (Math.max(...powerUsageData.map(item => parseInt(item.id.split('_')[1] || '0'))) + 1)
        : 'power_1';
  ...
  const handlePowerUsageMonthlyChange = (id: string, month: number, value: string) => {
  ...
  const handlePowerUsageFileChange = (id: string, file: File | null) => {
    setPowerUsageData((prev) =>
      prev.map((item) => (item.id === id ? { ...item, attachedFile: file } : item))
    );
  };
      ...
      setMeasurementData((prev) =>
        prev.map((item) => {
          if (item.id === id) {
            const updatedItem = { ...item, [field]: value };
            if (field === 'buildingId') {
              updatedItem.buildingName = dummyBuildings.find(b => b.id === value)?.name;
              updatedItem.processId = undefined; updatedItem.processName = undefined;
              updatedItem.subProcessId = undefined; updatedItem.subProcessName = undefined;
              updatedItem.facilityId = undefined; updatedItem.facilityName = undefined;
            } else if (field === 'processId') {
              updatedItem.processName = dummyProcesses.find(p => p.id === value)?.name;
              updatedItem.subProcessId = undefined; updatedItem.subProcessName = undefined;
              updatedItem.facilityId = undefined; updatedItem.facilityName = undefined;
            } else if (field === 'subProcessId') {
              updatedItem.subProcessName = dummySubProcesses.find(sp => sp.id === value)?.name;
              updatedItem.facilityId = undefined; updatedItem.facilityName = undefined;
            } else if (field === 'facilityId') {
              updatedItem.facilityName = dummyFacilities.find(f => f.id === value)?.name;
            }
            return updatedItem;
          }
          return item;
    ...
    <div className='p-4 bg-blue-50 border border-blue-200 rounded-md shadow'>
      ...
      {allocationSettings.hasMeasurement && measurementData.length === 0 && (
        <p className="text-xs text-green-600 mt-2">
          &quot;예&quot;를 선택하셨습니다. (2)번 테이블에 첫 번째 계측 항목이 추가되었습니다. 내용을 입력해주세요.
        </p>
      )}
    </div>
  );
