...
interface RefrigerantMobileFormProps {
  item: RefrigerantUtility;
...
const RefrigerantMobileForm: React.FC<RefrigerantMobileFormProps> = ({
  item, onInputChange, onFileChange, onDelete, isSelected, onSelect
}) => {
  const handleLocalInputChange = (field: keyof RefrigerantUtility, value: UtilityInputValue) => {
    ...
    >
      ...
      {isSelected && (
        <div className="mt-3 space-y-3">
          ...
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">물질명</label>
            <input 
              type="text" 
              value={item.materialName}
              onChange={(e) => handleLocalInputChange('materialName', e.target.value)}
          ...
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">CAS No.</label>
            <input 
              type="text" 
              value={item.casNumber}
              onChange={(e) => handleLocalInputChange('casNumber', e.target.value)}
              className="w-full p-2 border border-gray-300 rounded-md text-sm"
...
export default RefrigerantMobileForm; 