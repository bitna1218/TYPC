...
import { IndustrialGasUtility, UtilityInputValue } from './UtilitiesTabContent';

interface IndustrialGasDeskTopFormProps {
  items: IndustrialGasUtility[];
  onInputChange: (id: number, field: keyof IndustrialGasUtility, value: UtilityInputValue) => void;
  onFileChange: (id: number, file: File | null) => void;
  onDelete: (id: number) => void;
}

const IndustrialGasDeskTopForm: React.FC<IndustrialGasDeskTopFormProps> = ({ 
  items, onInputChange, onFileChange, onDelete 
}) => {
  if (!items || items.length === 0) {
    return <p className="text-center text-gray-500 py-4">산업용 가스 정보가 없습니다. &quot;산업용 가스 추가&quot; 버튼을 클릭하여 추가해주세요.</p>;
  }
...
export default IndustrialGasDeskTopForm; 